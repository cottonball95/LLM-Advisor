...
def get_conn():
    return psycopg2.connect(
        "postgresql://postgres:AIscheduler1!@db.lagyctcgaqulkpmflcjs.supabase.co:5432/postgres"
    )
...
with tab4:
    ...
    if programs:
        ...
        if not sections:
            ...
            section_dict = {s[0]: {"parent": s[1], "title": s[2], "rule": s[3],
                                   "n_req": s[4], "min_cr": s[5]} for s in sections}
            course_dict = {}
            for c in courses:
                course_dict.setdefault(c[3], []).append({
            ...
            def render_section(section_id, indent=0):
                ...
                rule_info += ")"
                st.markdown(f"{indent_str}**📂 {sec['title']}**{rule_info}")
